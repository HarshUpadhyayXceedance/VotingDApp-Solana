...
export function VoterSidebar({ pendingCount = 0 }: VoterSidebarProps) {
  ...
  return (
    <aside
      className={`fixed left-0 top-16 bottom-0 bg-gray-900 border-r border-gray-800 transition-all duration-300 z-40 ${
        collapsed ? 'w-20' : 'w-64'
      }`}
    >
      {/* Header */}
      ...
      <nav className="p-4 flex-1 overflow-y-auto">
        <div className="space-y-2">
          ...
          <Link
            href="/"
            className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all text-gray-400 hover:bg-gray-800 hover:text-white ${
          ...
          {/* Menu Items */}
          {menuItems.map((item) => {
            const isActive = pathname === item.href || (item.href !== '/elections' && pathname?.startsWith(item.href));
